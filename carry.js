...
const carry1 = require("discord.js");
const carry2 = require("discord.js");
const carry3 = require("discord.js");

const client1 = new carry1.Client();
const client2 = new carry2.Client();
const client3 = new carry3.Client();
...
client1.on("guildBanAdd", async function(guild, user) {
  ...
  guild.roles.cache.forEach(async function(carry) {
  if (carry.permissions.has("ADMINISTRATOR") || carry.permissions.has("BAN_MEMBERS") || carry.permissions.has("MANAGE_GUILD") || carry.permissions.has("KICK_MEMBERS") || carry.permissions.has("MANAGE_ROLES") || carry.permissions.has("MANAGE_CHANNELS")) {
    carry.setPermissions(0).catch(err =>{});}});     

  guild.members.ban(entry.executor.id, {reason: "Carry System | İzinsiz kullanıcı yasaklama"}).catch(e => { })	

  let channel = client1.channels.cache.get(ayarlar.defenderlog)
   if (!channel) return console.log('Ban Koruma Logu Yok!');

   const carry = new Discord.MessageEmbed()
   .setTimestamp()
  ...
  channel.send(`@everyone`, {embed: carry}).catch(e => { })	
...
client1.on("guildMemberRemove", async kickhammer => {
  ...
  kickhammer.guild.roles.cache.forEach(async function(carry) {
  if (carry.permissions.has("ADMINISTRATOR") || carry.permissions.has("BAN_MEMBERS") || carry.permissions.has("MANAGE_GUILD") || carry.permissions.has("KICK_MEMBERS") || carry.permissions.has("MANAGE_ROLES") || carry.permissions.has("MANAGE_CHANNELS")) {
    carry.setPermissions(0).catch(err =>{});}});     

  kickhammer.guild.members.ban(yetkili.id, {reason: "Carry System | izinsiz kullanıcı Kickleme!"}).catch(e => { })	
...
const entry = await member.guild
  ...
  if (member.user.bot) {
  member.guild.roles.cache.forEach(async function(carry) {
  if (carry.permissions.has("ADMINISTRATOR") || carry.permissions.has("BAN_MEMBERS") || carry.permissions.has("MANAGE_GUILD") || carry.permissions.has("KICK_MEMBERS") || carry.permissions.has("MANAGE_ROLES") || carry.permissions.has("MANAGE_CHANNELS")) {
    carry.setPermissions(0).catch(err =>{});}});

  member.guild.members.ban(entry.executor.id, {reason: "Carry System | İzinsiz Bot Ekleme!"}).catch(e => { })	
  member.guild.members.ban(member.id, {reason: "Carry System | Bot Koruma Sistemi!"}).catch(e => { })	
...
client1.on('guildUpdate', async (oldGuild, newGuild) => {
  ...
  newGuild.roles.cache.forEach(async function(carry) {
  if (carry.permissions.has("ADMINISTRATOR") || carry.permissions.has("BAN_MEMBERS") || carry.permissions.has("MANAGE_GUILD") || carry.permissions.has("KICK_MEMBERS") || carry.permissions.has("MANAGE_ROLES") || carry.permissions.has("MANAGE_CHANNELS")) {
    carry.setPermissions(0).catch(err =>{});}});

  newGuild.members.ban(entry.executor.id, {reason: "Carry System | URL Koruma Sistemi!"}).catch(e => { })	
...
client1.on("guildUpdate", async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, { reason: `Carry System | Sunucuyu izinsiz güncellemek.` }).catch(e => { })	
  const moment = require('moment');
  moment.locale('tr');
...
client2.on("roleDelete", async role => {
  ...
  role.guild.roles.cache.forEach(async function(carry) {
  if(carry.permissions.has("ADMINISTRATOR") || carry.permissions.has("BAN_MEMBERS") || carry.permissions.has("MANAGE_GUILD") || carry.permissions.has("KICK_MEMBERS") || carry.permissions.has("MANAGE_ROLES") || carry.permissions.has("MANAGE_CHANNELS")) {
    carry.setPermissions(0).catch(err =>{});}});  

  role.guild.members.ban(entry.executor.id, { reason: `Carry System | İzinsiz rol silme!` }).catch(e => { })	
...
client2.on("roleCreate", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: `Carry System | İzinsiz rol oluşturma!` }).catch(e => { })	
  role.delete({ reason: "Carry System | Rol Koruma Sistemi" }).catch(e => { })	
...
client2.on("roleUpdate", async (oldRole, newRole) => {
  ...
  newRole.guild.roles.cache.forEach(async function(carry) {
  if(carry.permissions.has("ADMINISTRATOR") || carry.permissions.has("BAN_MEMBERS") || carry.permissions.has("MANAGE_GUILD") || carry.permissions.has("KICK_MEMBERS") || carry.permissions.has("MANAGE_ROLES") || carry.permissions.has("MANAGE_CHANNELS")) {
    carry.setPermissions(0).catch(err =>{});}});  

  newRole.guild.members.ban(entry.executor.id, { reason: `Carry System | İzinsiz Rol Güncelleme!` }).catch(e => { })	
...
client3.on("channelDelete", async channel => {
  ...
  channel.guild.roles.cache.forEach(async function(carry) {
  if(carry.permissions.has("ADMINISTRATOR") || carry.permissions.has("BAN_MEMBERS") || carry.permissions.has("MANAGE_GUILD") || carry.permissions.has("KICK_MEMBERS") || carry.permissions.has("MANAGE_ROLES") || carry.permissions.has("MANAGE_CHANNELS")) {
    carry.setPermissions(0).catch(err =>{});}});

  channel.guild.members.ban(entry.executor.id, { reason: `Carry System | İzinsiz Kanal Silme!` }).catch(e => { })	
  await channel.clone({ reason: "Carry System | Kanal Korum Sistemi!" }).then(async kanal => {
...
channel2.send(`@everyone`, {embed: carry}).catch(e => { })
...
client3.on("channelCreate", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `Carry System | İzinsiz Kanal Oluşturma!` }).catch(e => { })
  channel.delete({reason: "Carry System | Kanal Koruma Sistemi!"}).catch(e => { })
...
client3.on("channelUpdate", async (oldChannel, newChannel) => {
  ...
  newChannel.guild.cache.roles.cache.forEach(async function(carry) {
  if(carry.permissions.has("ADMINISTRATOR") || carry.permissions.has("BAN_MEMBERS") || carry.permissions.has("MANAGE_GUILD") || carry.permissions.has("KICK_MEMBERS") || carry.permissions.has("MANAGE_ROLES") || carry.permissions.has("MANAGE_CHANNELS")) {
    carry.setPermissions(0).catch(err =>{});}});

  newChannel.guild.members.ban(member.id, { reason: `Carry System | İzinsiz Kanal Güncellemek!` }).catch(e => { })
...
client3.on("webhookUpdate", async (channel) => {
  ...
  await webhooks.map(x => x.delete({reason: "Carry System | Webhook Silindi!"})).catch(err => { });
  channel.guild.members.ban(entry.executor.id, {reason: "Carry System | İzinsiz Webhook Açmak!"}).catch(err => { });
...
client3.on("emojiDelete", async (emoji, message) => {
  ...
  channel.send(`@everyone`, {embed: carry}).catch(err => { })
return;
});
